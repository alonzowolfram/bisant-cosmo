...
# $folder will be the complete path. 
    ...
    if [ -d "$folder" ]; then
        ...
        # Create commands to run stitch-images and read-targets.
...
for i in `ls job.process.napari.${random_string}*`; do cat $i | bsub -o ${output_folder}/$i.lsf.out -e ${output_folder}/$i.lsf.err -q short -W 2:00 -M 80 -R 'rusage[mem=80]' -n 28 -u lwfong@mdanderson.org -J $i -P process_napari; done
